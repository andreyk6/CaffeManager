...
<#
.SYNOPSIS
Находит учетную запись хранения Microsoft Azure с именем "devtest*" в текущей подписке.
...
.PARAMETER  Location
Указывает расположение учетной записи хранения. Допустимыми значениями являются расположения Microsoft Azure, такие как "West US". Можно ввести расположение или территориальную группу, но не одновременно.
...
<#
.SYNOPSIS
Создает новую учетной записи хранения Microsoft Azure с уникальным именем, начинающимся с "devtest".

.DESCRIPTION
Функция Add-AzureVMStorage создает новую учетную запись хранения Microsoft Azure в текущей подписке. Имя учетной записи начинается с "devtest", за которым следует уникальная строка букв и цифр. Функция возвращает имя новой учетной записи хранения. Необходимо указать расположение или территориальную группу для новой учетной записи хранения.
...
function Add-AzureVMStorage
{
    ...
    # Создание новой учетной записи хранения Microsoft Azure с расположением/территориальной группой
...
function Read-ConfigFile
{
    ...
    # Определите, есть у объекта environmentSettings свойства cloudService (независимо от значения свойства)
    $hasCloudServiceProperty = Test-Member -Object $config.environmentSettings -Member 'cloudService'

    if (!$hasCloudServiceProperty)
    {
        throw 'Read-ConfigFile: файл конфигурации не содержит свойство cloudService.'
    }
...
<#
.SYNOPSIS
Создает все элементы новой виртуальной машины в подписке Microsoft Azure.

.DESCRIPTION
Эта функция создает виртуальную машину Microsoft Azure и возвращает URL-адрес развернутой виртуальной машины. Функция настраивает обязательные требования, а затем вызывает командлет New-AzureVM (модуль Azure) для создания новой виртуальной машины. 
...
— Вызывает командлет Add-AzureVM для создания новой виртуальной машины Microsoft Azure и возвращает новую виртуальную машину. Значения параметров функции обычно берутся из JSON-файла конфигурации, который Visual Studio создает для интегрированных с Microsoft Azure веб-проектов. Функция Read-ConfigFile в этом модуле преобразует JSON-файл в хэш-таблицу. Сохраняет ключ cloudservice или хэш-таблицу в переменной (как PSCustomObject) и использует свойства пользовательского объекта как значения параметров.
...
Указывает существующую службу Microsoft Azure или имя новой службы Microsoft Azure. Это имя передается в параметр ServiceName командлета New-AzureVM, который добавляет новую виртуальную машину в существующую службу Microsoft Azure или, если указано расположение или территориальная группа, создает новую виртуальную машину и службу в текущей подписке. Это обязательный параметр. 

.PARAMETER  ImageName
Указывает имя образа виртуальной машины для диска операционной системы. Этот параметр передается как значение параметра ImageName командлета New-AzureVMConfig. Это обязательный параметр. 
...
Указывает имя группы доступности для новой виртуальной машины. Когда в одну группу доступности помещено несколько виртуальных машин, Microsoft Azure пытается держать эти виртуальные машины на отдельных узлах для обеспечения непрерывности обслуживания в случае сбоя одного из них. Это необязательный параметр. 
...
.PARAMETER  Location
Указывает расположение для новой виртуальной машины. Допустимыми значениями являются расположения Microsoft Azure, такие как "West US". По умолчанию — расположение подписки. Это необязательный параметр. 

.PARAMETER  AffinityGroup
Указывает территориальную группу для новой виртуальной машины. Территориальная группа — это группа связанных ресурсов. При указании территориальной группы Microsoft Azure пытается держать ресурсы этой группы вместе для повышения эффективности. 
...
Подготавливает VM для развертывания. Это необязательный параметр. Если он не указан, VM создается, но не развертывается. Значение этого параметра включается в JSON-файл конфигурации, создаваемый Visual Studio для облачных служб.

.PARAMETER VMImage
Указывает, что ImageName является именем VMImage, а не OSImage. Это необязательный параметр. Если он не указан, ImageName будет использоваться как OSImage. Значение этого параметра включено в JSON-файл конфигурации, который Visual Studio создает для виртуальных машин.

.PARAMETER GeneralizedImage
Указывает, является ли состояние ОС универсальным для VMImage. Это необязательный параметр. Если он не указан, скрипт работает так же, как и в случае специализированного VMImage. Этот параметр не используется для образов OSImage. Значение этого параметра включено в JSON-файл конфигурации, который Visual Studio создает для виртуальных машин.
...
function Add-AzureVM
{
    ...
    # Создание нового объекта конфигурации Microsoft Azure.
    ...
    # VMImages сейчас не поддерживают расположение — новая VM будет создана в той же учетной записи хранения (расположении), в которой существует образ
...
<#
.SYNOPSIS
Получает указанную виртуальную машину Microsoft Azure.

.DESCRIPTION
Функция Find-AzureVM получает виртуальную машину Microsoft Azure по имени службы и имени виртуальной машины. Эта функция вызывает командлет Test-AzureName (модуль Azure), чтобы проверить существование имени службы в Microsoft Azure. Если имя существует, функция вызывает командлет Get-AzureVM для получения виртуальной машины. Функция возвращает хэш-таблицу с ключами vm и foundService.
...
.PARAMETER  ServiceName
Имя существующей службы Microsoft Azure. Это обязательный параметр.
...
Принимает хэш-таблицу с ключами Name и Password, например: @{Name = "admin"; Password = "password"} Это необязательный параметр. Если он опущен, по умолчанию используются имя пользователя виртуальной машины и пароль из JSON-файла конфигурации.
    ...
    if (!$findAzureVMResult.VM)
    {
        if(!$CloudServiceConfiguration.virtualMachine.isVMImage)
        {
            ...
            if (!$storageAccountName)
            {
             ...
             # Если учетной записи хранилища devtest* нет, создайте новую.
            if (!$storageAccountName)
            {
                if ($CloudServiceConfiguration.location)
                {
...
<#
.SYNOPSIS
Развертывает веб-пакет в Microsoft Azure.

.DESCRIPTION
Функция Publish-WebPackage использует MsDeploy.exe и ZIP-файл пакета веб-развертывания для развертывания ресурсов в веб-сайт Microsoft Azure. Эта функция не создает выходных данных. В случае сбоя вызова MSDeploy.exe функция создает исключение. Для получения более подробного вывода используйте общий параметр Verbose.
...
Разрешает ненадежные SSL-соединения с конечной точкой веб-развертывания на виртуальной машине. Этот параметр используется в вызове MSDeploy.exe. Это необязательный параметр.
...
<#
.SYNOPSIS
Развертывает виртуальную машину в Microsoft Azure.

.DESCRIPTION
Вспомогательная функция Publish-WebPackageToVM проверяет значения параметров, а затем вызывает функцию Publish-WebPackage.

.PARAMETER  VMDnsName
Указывает DNS-имя виртуальной машины Microsoft Azure. Это обязательный параметр.
